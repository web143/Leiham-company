...
export default function HeroLeiham({ isDark = true }: { isDark?: boolean }) {
    ...
    const subtitleY = useTransform(progress, [0, 0.4], [0, 30]);

    return (
        <section ref={sectionRef} style={{ height: isMobileHero ? '105vh' : '108vh', minWidth: isMobileHero ? undefined : '1100px' }} className={cn("relative transition-colors duration-300", isDark ? 'bg-black' : 'bg-white')}>
...
function FloatingProduct({
    ...
    const moveX = useTransform(progress, [0, 1.0], [0, scrollTarget.x]);
    const moveY = useTransform(progress, [0, 1.0], [0, scrollTarget.y]);
    const scaleOut = useTransform(progress, [0, 1.0], [1, scrollTarget.scale]);
    const opacityOut = useTransform(progress, [0, 0.95], [1, 0]);
