...
export default function HeroLeiham({ isDark = true }: { isDark?: boolean }) {
    ...
    const titleScale = useTransform(progress, [0, 1.0], [1, isMobileHero ? 3 : 8]);
    const titleOpacity = useTransform(progress, [0, 0.1, 0.8, 1.0], [1, 1, 1, 0]);
    const titleLetterSpacing = useTransform(progress, [0, 1.0], ["-0.02em", "0.3em"]);
    
    // Subtitle animations
    const subtitleOpacity = useTransform(progress, [0, 0.35], [1, 0]);
    const subtitleY = useTransform(progress, [0, 0.35], [0, 30]);

    return (
        <section ref={sectionRef} style={{ height: isMobileHero ? '110vh' : '115vh', minWidth: isMobileHero ? undefined : '1100px' }} className={cn("relative transition-colors duration-300", isDark ? 'bg-black' : 'bg-white')}>
...
function FloatingProduct({
    ...
    const moveX = useTransform(progress, [0, 0.95], [0, scrollTarget.x]);
    const moveY = useTransform(progress, [0, 0.95], [0, scrollTarget.y]);
    const scaleOut = useTransform(progress, [0, 0.95], [1, scrollTarget.scale]);
    const opacityOut = useTransform(progress, [0, 0.9], [1, 0]);
